...
import { Network } from '../src/client'
import { Wallet, providers } from 'ethers'
import { isValidMnemonic } from 'ethers/utils/hdnode'
import { BigNumber, parseEther, bigNumberify } from 'ethers/utils'
import { TransactionResponse, TransactionReceipt } from 'ethers/providers/abstract-provider'
...
describe('Wallets', () => {
  ...
  it('should create a wallet from phrase', () => {
    ...
    expect(ethClient.wallet['signingKey']).toMatchObject(wallet.signingKey)
  })

  it('should set new phrase', () => {
    const ethClient = new Client()
    const newWallet = ethClient.setPhrase(newPhrase)
    expect(ethClient.wallet.mnemonic).toEqual(newPhrase)
...
describe('Utils', () => {
  it('should generate a phrase', () => {
    ...
    expect(isValidMnemonic(newPhrase)).toBeTruthy()
  })

  it('should get address', () => {
    const ethClient = new Client(Network.TEST, phrase)
    expect(ethClient.address).toEqual(address)
  })
...
describe('Transactions', () => {
  ...
  it('sends a normalTx', async () => {
    ...
    await ethClient.normalTx(ethClient.address, parseEther('1'))
...
describe('Balances', () => {
  it('gets a balance without address args', async () => {
    ...
    mockBalance.mockImplementation(async (_): Promise<BigNumber> => Promise.resolve(parseEther('1.786')))
  ...
  it('gets a balance from address', async () => {
    ...
    const mockBalance = jest.spyOn(ethClient.wallet.provider, 'getBalance')
    mockBalance.mockImplementation(async (_): Promise<BigNumber> => Promise.resolve(parseEther('0.1')))
