...
- 766cf9d: **Breaking (Chainflip):** `estimateSwap` no longer opens a deposit channel.
  ...
  **Migration:**
  ...
  1. Use `estimateSwap` for quote refresh only (`toAddress` is `''` and `depositChannelId` is `undefined` for Chainflip; `canSwap` means a usable quote exists — not “ready to send”).
...
- 2e28cb5: Point mainnet THORChain public defaults at the Liquify gateway after `*.thorchain.network` hosts stopped resolving (DNS failure for `rpc`, `thornode`, and `midgard`).
...
- Updated dependencies [2e28cb5]
  ...
  - @xchainjs/xchain-mayachain@4.2.0
...
- 5f2e0d7: Chainflip: upgrade @chainflip/sdk to 2.2.1 and add Tron support
