...
describe('BitcoinClient Test', () => {
  ...
  it('should broadcast a normal transfer', async () => {
    ...
    const txid = await btcClient.transfer({ recipient: addyTwo, amount: 2223, feeRate: 1 })
  ...
  it('should do broadcast a vault transfer with a memo', async () => {
    ...
    const txid = await btcClient.transfer({ recipient: addyOne, amount: 2223, memo: MEMO, feeRate: 1 })
    expect(txid).toEqual(expect.any(String))
  })
  ...
  it('should prevent a tx when fees and valueOut exceed balance', async () => {
    ...
    btcClient.setPhrase(phraseOne)
    expect(
      async () => await btcClient.transfer({ recipient: addyTwo, amount: 9999999999, feeRate: 1 }),
  ...
  it('should error when an invalid address is provided', async () => {
    ...
    expect(
      async () => await btcClient.transfer({ recipient: invalidAddress, amount: 99000, feeRate: 1 }),
