...
const defaultMayanode = new Mayanode()
...
export class MayachainCache {
  ...
  private async refereshInboundDetailCache(): Promise<void> {
    ...
    inboundDetails[MAYAChain] = details
  ...
  private async refereshNetworkValuesCache(): Promise<void> {
    const networkValues = await this.mayanode.getNetworkValues()
