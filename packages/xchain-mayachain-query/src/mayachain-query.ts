...
const defaultCache = new MayachainCache()
...
export class MayachainQuery {
  ...
  /**
   * Contructor to create a MayachainAMM
   *
   ...
   */
  constructor(mayachainCache = defaultCache, chainAttributes = DefaultChainAttributes) {
  ...
  public async quoteSwap({
    ...
    // fetch quote
    const swapQuote = await this.mayachainCache.mayanode.getSwapQuote(
      fromAssetString,
      toAssetString,
    ...
    if (response.error) errors.push(`Mayanode request quote: ${response.error}`)
    if (errors.length > 0) {
      return {
        memo: ``,
        toAddress: ``,
        dustThreshold: new CryptoAmount(baseAmount(0), AssetCacao),
        expiry: new Date(),
        txEstimate: {
          totalFees: {
            asset: destinationAsset,
            affiliateFee: new CryptoAmount(baseAmount(0), AssetCacao),
            outboundFee: new CryptoAmount(baseAmount(0), AssetCacao),
          },
          slipBasisPoints: 0,
          ...
          canSwap: false,
          errors,
          netOutputStreaming: new CryptoAmount(baseAmount(0), AssetCacao),
  ...
  async confCounting(inbound: CryptoAmount): Promise<number> {
    ...
    if (
      isAssetCacaoNative(inbound.asset) ||
    ...
    const amountInGasAsset = await this.mayachainCache.convert(inbound, chainGasAsset)
  ...
  public async estimateAddLP(params: AddliquidityPosition): Promise<EstimateAddLP> {
    ...
    let runeInboundFee = new CryptoAmount(baseAmount(0), AssetCacao)
  ...
  public async checkLiquidityPosition(asset: Asset, assetOrRuneAddress?: string): Promise<LiquidityPosition> {
    const poolAsset = await this.mayachainCache.getPoolForAsset(asset)
    ...
    const liquidityProvider = await this.mayachainCache.mayanode.getLiquidityProvider(
    ...
    const blockData = (await this.mayachainCache.mayanode.getLastBlock()).find((item) => item.chain === asset.chain)
    ...
    const networkValues = await this.mayachainCache.mayanode.getNetworkValues()
    const block: Block = {
      current: blockData.mayachain,
  ...
  public async estimateWithdrawLP(params: WithdrawLiquidityPosition): Promise<EstimateWithdrawLP> {
    ...
    const allInboundDetails = await this.mayachainCache.getInboundDetails()
    const inboundDetails = allInboundDetails[params.asset.chain]
    const runeInbound = calcNetworkFee(AssetCacao, inboundDetails)
    const assetInbound = calcNetworkFee(params.asset, inboundDetails)
    const runeOutbound = calcOutboundFee(AssetCacao, inboundDetails)
  ...
  public async estimateAddSaver(addAmount: CryptoAmount): Promise<EstimateAddSaver> {
    ...
    // Fetch quote
    const depositQuote = await this.mayachainCache.mayanode.getSaversDepositQuote(
    ...
    const pool = (await this.mayachainCache.getPoolForAsset(addAmount.asset)).pool
    // Organise fees
    ...
    const estimateAddSaver: EstimateAddSaver = {
      ...
      estimatedDepositValue: new CryptoAmount(baseAmount(depositQuote.expected_amount_out), addAmount.asset),
      fee: saverFees,
  ...
  private async getAddSaversEstimateErrors(addAmount: CryptoAmount): Promise<string[]> {
    const errors = []
    const pools = await this.mayachainCache.getPools()
