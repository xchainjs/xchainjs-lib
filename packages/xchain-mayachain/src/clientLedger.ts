...
import { Client, MayachainClientParams } from './client'
import { AssetCacao, DEPOSIT_GAS_LIMIT_VALUE, defaultClientConfig } from './const'
import { DepositParam, TxParams } from './types'
import { parseAssetToMayanodeAsset, parseDerivationPath, sortAndStringifyJson, sortedObject } from './utils'

export class ClientLedger extends Client {
  private app: THORChainApp

  constructor(params: MayachainClientParams & { transport: Transport }) {
  ...
  /**
   * Transfers CACAO or synth token
   *
   ...
   * @param {asset} params.asset Optional - The asset that will be deposit. If it is not set, Mayachain native asset will be
   * used
   ...
   * If it is not set, address associated with index 0 will be used
   * @param {Asset} param.asset Optional - The asset that will be deposit. If it is not set, Mayachain native asset will be
   * used
          ...
          amount: [
            {
              ...
              denom: this.getDenom(params.asset || AssetCacao),
            },
          ],
        },
      },
      ...
      '/types.MsgDeposit': {
        ...
        fromAmino: (params) => ({
          ...
          coins: params.coins.map((coin: { asset: string; amount: string }) => {
            return {
              ...coin,
              asset: parseAssetToMayanodeAsset(assetFromStringEx(coin.asset)),
            }
          }),
        }),
      },
