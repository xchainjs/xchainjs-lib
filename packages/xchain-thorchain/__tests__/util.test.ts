  ...
  describe('transaction util', () => {
    ...
    describe('getDepositTxDataFromLogs', () => {
      it('returns data for IN tx (SWAP RUNE -> BTC)', () => {
        ...
        const { from, to, type } = getDepositTxDataFromLogs(tx.logs)

        expect(from[0].amount.amount().toString()).toEqual(assetToBase(assetAmount('36000')).amount().toString())
        expect(from[0].from).toEqual('thor1g3nvdxgmdte8cfhl8592lz5tuzjd9hjsglazhr')
        expect(to[0].to).toEqual('thor1g98cy3n9mmjrpn0sxmn63lztelera37n8n67c0')
      ...
      it('returns data for SEND tx', () => {
        ...
        expect(from[0].amount.amount().toString()).toEqual(assetToBase(assetAmount(5)).amount().toString())
        expect(from[0].from).toEqual('thor1ws0sltg9ayyxp2777xykkqakwv2hll5ywuwkzl')
        expect(to[0].to).toEqual('thor1mryx88xxhvwu9yepmg968zcdaza2nzz4rltjcp')
        expect(type).toEqual('transfer')
      })
      ...
      it('getDepositTxDataFromLogs', () => {
        ...
        expect(from[0].amount.amount().toString()).toEqual(assetToBase(assetAmount(1700)).amount().toString())
        expect(from[0].from).toEqual('tthor137kees65jmhjm3gxyune0km5ea0zkpnj4lw29f')
        expect(to[0].to).toEqual('tthor17gw75axcnr8747pkanye45pnrwk7p9c3uhzgff')
        expect(type).toEqual('transfer')
      })
    })
