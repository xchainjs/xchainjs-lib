...
LedgerBtc.prototype.getWalletPublicKey_async = function (path) {
  ...
  return this.comm.exchange(buffer.toString('hex'), [0x9000]).then(function (response) {
    ...
    result['bitcoinAddress'] = response
...
LedgerBtc.prototype.getTrustedInput_async = function (indexLookup, transaction) {
  ...
  var processScriptBlocks = function (script, sequence) {
    ...
    while (offset != script.length) {
      var blockSize =
        script.length - offset > LedgerBtc.MAX_SCRIPT_BLOCK ? LedgerBtc.MAX_SCRIPT_BLOCK : script.length - offset
    ...
    async.eachSeries(
      ...
      function (scriptBlock, finishedCallback) {
        currentObject
          .getTrustedInputRaw_async(false, undefined, scriptBlock)
  ...
  var processInputs = function () {
    async.eachSeries(
      ...
      function (input, finishedCallback) {
        ...
        currentObject
          .getTrustedInputRaw_async(false, undefined, data)
          .then(function (result) {
  ...
  currentObject
    .getTrustedInputRaw_async(true, indexLookup, data)
    .then(function (result) {
      processInputs()
    })
    .fail(function (err) {
...
LedgerBtc.prototype.startUntrustedHashTransactionInputRaw_async = function (
  newTransaction,
  firstRound,
  transactionData,
) {
...
LedgerBtc.prototype.startUntrustedHashTransactionInput_async = function (newTransaction, transaction, inputs) {
  ...
  var deferred = Q.defer()
  currentObject
    .startUntrustedHashTransactionInputRaw_async(newTransaction, true, data)
    .then(function (result) {
      var i = 0
      async.eachSeries(
        transaction['inputs'],
        function (input, finishedCallback) {
          ...
          currentObject
            .startUntrustedHashTransactionInputRaw_async(newTransaction, false, data)
            .then(function (result) {
              var scriptBlocks = []
              var offset = 0
              if (input['script'].length == 0) {
                ...
                while (offset != input['script'].length) {
                  var blockSize =
                    input['script'].length - offset > LedgerBtc.MAX_SCRIPT_BLOCK
                      ? LedgerBtc.MAX_SCRIPT_BLOCK
              ...
              async.eachSeries(
                ...
                function (scriptBlock, blockFinishedCallback) {
                  currentObject
                    .startUntrustedHashTransactionInputRaw_async(newTransaction, false, scriptBlock)
                    .then(function (result) {
...
LedgerBtc.prototype.hashOutputFull_async = function (outputScript) {
  ...
  return LedgerBtc.asyncWhile(
    function () {
      return offset < outputScript.length
    },
    function () {
      var blockSize =
        offset + LedgerBtc.MAX_SCRIPT_BLOCK >= outputScript.length
...
LedgerBtc.prototype.createPaymentTransactionNew_async = function (
  inputs,
  associatedKeysets,
  changePath,
  outputScript,
  ...
  LedgerBtc.foreach(inputs, function (input, i) {
    ...
    .then(function () {
      return LedgerBtc.foreach(inputs, function (input, i) {
        ...
        return self
          .startUntrustedHashTransactionInput_async(firstRun, targetTransaction, trustedInputs)
          .then(function () {
            return LedgerBtc.doIf(!resuming && typeof changePath != 'undefined', function () {
              ...
              .then(function (resultHash) {
                return self
                  .signTransaction_async(associatedKeysets[i], lockTime, sigHashType)
...
LedgerBtc.prototype.signP2SHTransaction_async = function (
  inputs,
  associatedKeysets,
  outputScript,
  lockTime,
  ...
  for (var i = 0; i < transaction['inputs'].length; i++) {
    ...
    console.log(
      'input ' +
        i +
        ' prevout ' +
        input['prevout'].toString('hex') +
        ' script ' +
        input['script'].toString('hex') +
        ' sequence ' +
        input['sequence'].toString('hex'),
    )
  }
  for (var i = 0; i < transaction['outputs'].length; i++) {
    var output = transaction['outputs'][i]
    console.log(
      'output ' + i + ' amount ' + output['amount'].toString('hex') + ' script ' + output['script'].toString('hex'),
    )
  }
