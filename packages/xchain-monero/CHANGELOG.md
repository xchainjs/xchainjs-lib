# @xchainjs/xchain-monero

## 0.2.7

### Patch Changes

- 0b77252: Stop using the in-process RingCT builder for `transfer` (wallet-rpc only), checksum-validate addresses, point stagenet at stagenet explorers/daemons, and scale fee estimates by typical tx weight.

## 0.2.6

### Patch Changes

- Updated dependencies [322b1bf]
- Updated dependencies [2e28cb5]
  - @xchainjs/xchain-client@2.0.17

## 0.2.5

### Patch Changes

- Updated dependencies [4ec2e3e]
  - @xchainjs/xchain-crypto@1.0.8
  - @xchainjs/xchain-client@2.0.16

## 0.2.4

### Patch Changes

- Updated dependencies [dbdfc76]
  - @xchainjs/xchain-crypto@1.0.7
  - @xchainjs/xchain-client@2.0.15

## 0.2.3

### Patch Changes

- Updated dependencies [51569ce]
  - @xchainjs/xchain-client@2.0.14

## 0.2.2

### Patch Changes

- Updated dependencies [70acc68]
- Updated dependencies [5f92a68]
  - @xchainjs/xchain-client@2.0.13

## 0.2.1

### Patch Changes

- Updated dependencies [0246a01]
- Updated dependencies [c4682c4]
  - @xchainjs/xchain-client@2.0.12
  - @xchainjs/xchain-util@2.0.7

## 0.2.0

### Minor Changes

- 73ec2d1: Add Monero (XMR) chain client package with BIP-39 key derivation, daemon RPC integration, and address validation

### Patch Changes

- Updated dependencies [3dd82dd]
  - @xchainjs/xchain-util@2.0.6
  - @xchainjs/xchain-client@2.0.11
