...
describe('ThorchainAMM', () => {
  describe('validateSwap (affiliates)', () => {
    ...
    it('forwards the affiliates array to the quote endpoint as slash-joined params', async () => {
      ...
      const quote = await thorchainAMM.estimateSwap({
        ...
        affiliates: [
          { address: 'odin', bps: 10 },
          { address: 'thor1s76zxv0kpr78za293kvj0eep4tfqljacknsjzc', bps: 20 },
        ],
      })

      expect(quote.txEstimate.canSwap).toBe(true)
      const quoteRequest = mockAdapter.history.get.find((request) => `${request.url}`.includes('/thorchain/quote/swap'))
      expect(quoteRequest).toBeDefined()
      const params = new URL(`${quoteRequest?.url}`).searchParams
      expect(params.get('affiliate')).toBe('odin/thor1s76zxv0kpr78za293kvj0eep4tfqljacknsjzc')
      expect(params.get('affiliate_bps')).toBe('10/20')
    })
  })
})
