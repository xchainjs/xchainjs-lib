...
export class Wallet extends Signer implements ExternallyOwnedAccount, TypedDataSigner {
  ...
  static async fromMnemonic(mnemonic: string, path?: string, wordlist?: Wordlist): Promise<Wallet> {
    if (!path) {
      path = defaultPath
    }
    return new Wallet(await (await HDNode.fromMnemonic(mnemonic, null, wordlist)).derivePath(path))
  }
}
