...
const AVAX_ONLINE_PROVIDER_MAINNET = new CovalentProvider(API_KEY, AssetAVAX.chain, 43114, AssetAVAX, 18)
const AVAX_ONLINE_PROVIDER_TESTNET = new CovalentProvider(API_KEY, AssetAVAX.chain, 43113, AssetAVAX, 18)
...
describe('Client Test', () => {
  ...
  describe('#constructor', () => {
    it('should throw error on bad phrase', () => {
      expect(() => {
        new Client({ ...avaxParams, phrase: 'bad phrase' })
      }).toThrowError()
    })

    it('should not throw on a client without a phrase', () => {
      const params = { ...avaxParams }
  ...
  describe('#purgeClient', () => {
    it('should not have a phrase after purging', () => {
      ...
      expect(avaxClient['phrase']).toEqual(phrase)
      avaxClient.purgeClient()
      expect(avaxClient['phrase']).toEqual('')
    })
  })
  ...
  describe('#getWallet', () => {
    ...
    it('should create a wallet from phrase', () => {
      expect(avaxClient.getWallet(0)).toBeInstanceOf(Wallet)
  ...
  it('should set new phrase', () => {
    const avaxClient = new Client(avaxParams)
    const newWallet = avaxClient.setPhrase(newPhrase)
    expect(avaxClient.getWallet(0).mnemonic.phrase).toEqual(newPhrase)
  ...
  it('should connect to specified network', async () => {
    const avaxClient = new Client({ ...avaxParams, network: Network.Mainnet })
  ...
  //   const txHistory = await avaxClient.getTransactions({
  //     address,
  //     limit: 1,
  //   })
