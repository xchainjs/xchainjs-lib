...
import { Shield, Check, Search, Ban } from 'lucide-react'
...
export default function RouterApprovalPage() {
  ...
  const { thorchainAmm, thorchainQuery, wallet, loading, error } = useLiquidity()
  ...
  const revokeOp = useOperation<ApprovalResult>()
  const allowanceOp = useOperation<AllowanceResult>()
  const { client: evmClient } = useChainClient(selectedChain)
  ...
  const handleCheckAllowance = async () => {
    if (!evmClient || !wallet) return

    await allowanceOp.execute(
      async () => {
        // Get the router address from the appropriate protocol's query
        let routerAddress: string
        if (protocol === 'THORChain') {
          if (!thorchainQuery) throw new Error('THORChain Query not initialized')
          const inbound = await thorchainQuery.getChainInboundDetails(selectedChain)
          if (!inbound.router) throw new Error(`No router address for ${selectedChain}`)
          routerAddress = inbound.router
        } else {
          const { MayachainQuery } = await import('@xchainjs/xchain-mayachain-query')
          const mayaQuery = new MayachainQuery()
          const inbound = await mayaQuery.getChainInboundDetails(selectedChain)
          if (!inbound.router) throw new Error(`No router address for ${selectedChain}`)
          routerAddress = inbound.router
        }
      ...
      { operation: 'checkAllowance', params: { token: selectedToken.symbol, chain: selectedChain, protocol } },
    )
  }
  ...
  const handleApprove = async () => {
    ...
    await approveOp.execute(
      async () => {
        ...
        const cryptoAmount = new CryptoAmount(assetToBase(assetAmount(amountNum, selectedToken.decimals)), tokenAsset)
  ...
  const handleRevoke = async () => {
    ...
    await revokeOp.execute(
      async () => {
        ...
        // Pass baseAmount(0) — sets allowance to zero (revoke)
        const zeroAmount = new CryptoAmount(assetToBase(assetAmount(0, selectedToken.decimals)), tokenAsset)

        if (protocol === 'THORChain') {
          if (!thorchainAmm) throw new Error('THORChain AMM not initialized')
          return await thorchainAmm.approveRouterToSpend({ asset: tokenAsset, amount: zeroAmount })
        } else {
          const { MayachainAMM } = await import('@xchainjs/xchain-mayachain-amm')
          const { MayachainQuery } = await import('@xchainjs/xchain-mayachain-query')
          const mayachainAmm = new MayachainAMM(new MayachainQuery(), wallet)
          return await mayachainAmm.approveRouterToSpend({ asset: tokenAsset, amount: zeroAmount })
        }
      },
      { operation: 'revokeRouter', params: { token: selectedToken.symbol, protocol } },
    )
  }
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <div className="text-center py-8">
              ...
              <p className="text-gray-500 dark:text-gray-400">Please connect your wallet to approve router spending.</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow">
            ...
            <div className="p-6 space-y-6">
              <div className="space-y-4">
                ...
                {/* Chain Selection */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Chain</label>
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-gray-500 dark:text-gray-400 font-mono">{selectedToken.address}</p>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <button
                    onClick={() => void handleCheckAllowance()}
                    disabled={allowanceOp.loading || !evmClient}
                    className="px-4 py-2.5 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  >
                    {allowanceOp.loading ? (
                      <>
                  ...
                  <button
                    ...
                    className="px-4 py-2.5 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  >
                    {approveOp.loading ? (
                      <>
                        <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
                ...
                {/* Allowance Result */}
                <ResultPanel loading={allowanceOp.loading} error={allowanceOp.error} duration={allowanceOp.duration}>
                  {allowanceOp.result && (
                    <div className="space-y-1">
                      <span className="text-sm text-gray-500 dark:text-gray-400">
                        Current {allowanceOp.result.token} allowance for {protocol} router:
                      </span>
                      <p className="font-mono text-lg text-gray-900 dark:text-gray-100">
                        {allowanceOp.result.formatted} {allowanceOp.result.token}
                      </p>
                      <p className="font-mono text-xs text-gray-400 dark:text-gray-500 break-all">
                        Raw: {allowanceOp.result.allowance}
                      </p>
                    </div>
                  )}
                </ResultPanel>
              ...
              {/* Revoke Result */}
              <ResultPanel loading={revokeOp.loading} error={revokeOp.error} duration={revokeOp.duration}>
                {revokeOp.result && (
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 text-red-700 dark:text-red-300">
                      <Ban className="w-5 h-5" />
                      <span className="font-medium">Allowance Revoked!</span>
                    </div>
                    <div>
                      <span className="text-gray-500 dark:text-gray-400 text-sm">Transaction Hash:</span>
                      <p className="font-mono text-sm text-gray-900 dark:text-gray-100 break-all">
                        {revokeOp.result.hash}
                      </p>
                    </div>
                    {revokeOp.result.url && (
                      <a
                        href={revokeOp.result.url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-blue-600 dark:text-blue-400 hover:underline text-sm"
                      >
