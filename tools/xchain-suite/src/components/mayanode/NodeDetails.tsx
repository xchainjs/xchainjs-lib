...
export function NodeDetails({ nodesApi }: NodeDetailsProps) {
  ...
  const handleFetchNode = async () => {
    ...
    await execute(async () => {
      ...
      return {
        ...
        status: node.status,
        operatorAddress: node.bond_address,
        totalBond: baseToAsset(baseAmount(node.bond, 10)).amount().toFormat(2) + ' CACAO',
        bondProviders: (node.bond_providers?.providers || []).map((p: any) => ({
          address: p.bond_address,
          bond: baseToAsset(baseAmount(p.bond || '0', 10)).amount().toFormat(2) + ' CACAO',
        })),
        operatorFee: (Number(node.bond_providers?.node_operator_fee || '0') / 100).toFixed(2) + '%',
  ...
  const codeExample = `import { NodesApi, Configuration, MAYANODE_API_URL } from '@xchainjs/xchain-mayanode'

// Initialize API
const config = new Configuration({ basePath: MAYANODE_API_URL })
...
console.log('Bond Address:', node.bond_address)
console.log('Status:', node.status)
console.log('Bond:', node.bond)
    ...
    <div className="space-y-6">
      <div>
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
          Query detailed information about a specific MAYAChain validator node.
        </p>
      </div>
